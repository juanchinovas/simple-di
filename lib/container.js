...
exports.provide = exports.getContainer = void 0;
...
class Container {
    ...
    register({ target, name, dependencies = [], scope = index_1.MetadataScope.transient }) {
        if (name && ["string", "symbol"].includes(typeof name) && target && dependencies.length === 0) {
            this.#instances.set(name, target);
            return true;
        }
        if (target && target.name && Array.isArray(dependencies)) {
            ...
            (0, common_1.defineMetadata)("class:construct:binded:params" /* BindedKey.bindedParams */, dependencies.map((dep, i) => ({
                target: dep.name ?? dep,
                paramIndex: i,
            })), _target);
            common_1.mappedKey.set(name ?? _target.name, _target);
            return true;
        }
        return false;
    }
